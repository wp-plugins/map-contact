...
function shortcodeManagment($attributes){
    ...
    if ($attributes["addressbook"]=="true")
    {
        ...
        foreach ($addresses as $address) {
            $address = get_object_vars($address);
            if (!empty($address["email"])) { $contact = '<button type="button" onclick="document.getElementById(\''.$address["id"].'_lightbox\').style.display=\'inline\';" >Contact by email</button>'; }
